...
def rendering_romp_bev_results(renderer, outputs, image, rendering_cfgs, alpha=1):
    ...
    if rendering_cfgs['renderer'] == 'sim3dr':
        ...
        if 'mesh_only' in rendering_cfgs['items']:
            rendered_image = renderer(vertices, triangles, background, mesh_colors=mesh_colors)
            result_image=rendered_image
        elif 'mesh' in rendering_cfgs['items']:
    ...
    if 'mesh_only' in rendering_cfgs['items']:
        outputs['rendered_image'] = result_image
    else:     
