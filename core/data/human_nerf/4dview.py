...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.train_mesh_info = self.mesh_infos[framelist[self.train_frame_idx]]

        self.bgcolor = bgcolor if bgcolor is not None else [255., 255., 255.]
        self.keyfilter = keyfilter
        self.src_type = src_type

    # change the frame index to construct the outputs
    def change_frame_index(self,train_frame_idx):
        self.train_frame_idx = train_frame_idx
        # print(f' -- Frame Idx: {self.train_frame_idx}')
    ...
    def get_4d_camera(self,frame_idx, total_frames, trans=None):
        self.change_frame_index(frame_idx)
        E = rotate_camera_by_frame_idx(
                extrinsics=self.train_camera['extrinsics'], 
                frame_idx=frame_idx,
                period=total_frames,
    ...
    def __getitem__(self, idx):
        ...
        # print("idx",idx)
        # print("self frame idx",self.train_frame_idx)
        
        K, E = self.get_4d_camera(
                        frame_idx=idx,
